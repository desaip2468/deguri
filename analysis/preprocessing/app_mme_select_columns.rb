...
dst_filename = 'refine_categorytime_mme.csv'
...
if File.exist?('all_dst_file_sub_categories') && File.exist?('dst_hash')
  ...
  CSV.foreach(dst_filename, headers: true) do |row|
    ...
    all_dst_file_sub_categories << row['sub_category']
    
    dst_hash[panel_id] ||= Hash.new 0
    dst_hash[panel_id][row['sub_category']] += row['Total_UsageTime'].to_i
...
File.write('payments_ppdb_app.csv', output)
