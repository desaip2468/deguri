...
table(old$cluster)
old %>% select(cluster) %>% filter(cluster>0)

old %>% group_by(cluster) %>%  summarise(age=mean(age),gender=mean(gender))
##나이 적을수록, 남자일수록 heavy user

analy<-aov(age~cluster,data=user)
summary(analy)
#일원배치 분산분석
bartlett.test(age~cluster,data=user)
#오차의 등분산성 가정 만족안함 

analy<-aov(gender~cluster,data=user)
summary(analy)
#일원배치 분산분석
bartlett.test(age~cluster,data=user)
#오차의 등분산성 가정 만족안함 
...
#heavy user~light user분류가 의미있음을 다시한번 알수있음 
